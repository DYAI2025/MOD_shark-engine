...
- **Directory**: [`hovercraft-controller/`](hovercraft-controller/)
- **Technology**: Java 21 (no MC/Fabric dependencies)
- **Responsibility**: Pure flight logic — movement vector computation, deceleration, speed capping. Testable with JUnit 5 without Fabric.

### Client Input

- **Directory**: [`client-input/`](client-input/)
- **Technology**: Java 21 + Fabric Client API (Loom client source set)
- **Responsibility**: Keyboard/gamepad input capture, deadzone filtering, normalization, HovercraftInputPayload transmission.

### Server Integration

- **Directory**: [`server-integration/`](server-integration/)
- **Technology**: Java 21 + Fabric Server API
- **Responsibility**: ShipEntity delegation to HovercraftController, C2S payload reception/validation, BUG-block decoupling.
