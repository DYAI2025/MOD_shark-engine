...
// ──────────────────────────────────────────────
server.registerTool("modrinth_search_projects", {
    title: "Search Modrinth Projects",
    description: `Search for mods, modpacks, resource packs, or shaders on Modrinth.
...
// ──────────────────────────────────────────────
server.registerTool("modrinth_get_project", {
    title: "Get Modrinth Project",
    description: `Get full details of a Modrinth project by ID or slug.
...
Returns: title, description, body (markdown), categories, game versions, loaders, download/follower counts, links, license.`,
    ...
    try {
        const p = await apiGet(`/project/${encodeURIComponent(id_or_slug)}`);
        const lines = [
            `# ${p.title} (${p.slug})`,
            "",
        ...
        if (p.issues_url)
            lines.push(`**Issues**: ${p.issues_url}`);
        if (p.discord_url)
            lines.push(`**Discord**: ${p.discord_url}`);
        return { content: [{ type: "text", text: lines.join("\n") }] };
    }
    catch (e) {
        return { content: [{ type: "text", text: handleError(e) }], isError: true };
...
// ──────────────────────────────────────────────
server.registerTool("modrinth_list_versions", {
    title: "List Project Versions",
    description: `List all versions of a Modrinth project, with optional filters for loader and game version.
...
Returns: version name, number, type (release/beta/alpha), game versions, loaders, download count, file URLs.`,
    ...
    try {
        ...
        const versions = await apiGet(`/project/${encodeURIComponent(id_or_slug)}/version`, params);
...
// ──────────────────────────────────────────────
server.registerTool("modrinth_get_version", {
    title: "Get Version Details",
    description: `Get full details of a specific version by its ID. Includes changelog, files with hashes, and dependencies.`,
    inputSchema: {
    ...
    try {
        const v = await apiGet(`/version/${encodeURIComponent(version_id)}`);
        const lines = [
...
// ──────────────────────────────────────────────
server.registerTool("modrinth_create_version", {
    title: "Create Version (Upload)",
    description: `Upload a new version to a Modrinth project. Requires MODRINTH_TOKEN with VERSION_CREATE scope.
...
Args:
  ...
  - dependencies: Optional JSON array of {project_id, dependency_type} objects`,
    ...
    try {
        ...
        const version = await apiPostMultipart("/version", form);
...
// ──────────────────────────────────────────────
server.registerTool("modrinth_modify_project", {
    title: "Modify Project",
    description: `Update project settings on Modrinth. Requires MODRINTH_TOKEN with PROJECT_WRITE scope.
...
// ──────────────────────────────────────────────
server.registerTool("modrinth_list_game_versions", {
    ...
    try {
        let versions = await apiGet("/tag/game_version");
        if (type_filter !== "all") {
...
// ──────────────────────────────────────────────
server.registerTool("modrinth_list_loaders", {
    ...
    try {
        const loaders = await apiGet("/tag/loader");
...
// ──────────────────────────────────────────────
server.registerTool("modrinth_list_categories", {
    ...
    try {
        let categories = await apiGet("/tag/category");
        if (project_type !== "all") {
...
// ──────────────────────────────────────────────
server.registerTool("modrinth_get_user_projects", {
    ...
    try {
        const projects = await apiGet(`/user/${encodeURIComponent(user)}/projects`);
